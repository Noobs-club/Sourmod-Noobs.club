...
public Plugin:myinfo =
{
	...
	author = "stachi, modified by WhiteWolf",
...
bool:IsCamping(client)
{
	...
	new bool:Location = StrEqual(place, "TSpawn", true);
	
	if(GetVectorDistance(g_fLastPos[client], CurrentPos) < GetConVarInt(g_CvarRadius))
	{
		if(!g_bIsAfk[client])
			if((GetClientHealth(client) > GetConVarInt(g_CvarMinHealth) || GetConVarBool(g_CvarPunishAnyway))&& Location)
				return true;
	}
